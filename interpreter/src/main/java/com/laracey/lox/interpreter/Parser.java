...
import static com.laracey.lox.interpreter.TokenType.*;
...
public class Parser {
  ...
  private Stmt forStatement() {
    ...
    // For-loops are implemented by desugaring into a while loop.
  ...
  private Stmt expressionStatement() {
    ...
    consume(SEMICOLON, "Exect ';' after expression.");
  ...
  private Expr and() {
    Expr expr = ternary(); // equality() in the book.

    while (match(AND)) {
      Token operator = previous();
      Expr right = ternary(); // equality() in the book.
      expr = new Expr.Logical(expr, operator, right);
    }
  ...
  private Expr ternary() {
    ...
    while (match(QUESTION_MARK)) {
      Expr middle = ternary();
      consume(COLON, "Expect ':' for ternary");
      Expr right = ternary();
